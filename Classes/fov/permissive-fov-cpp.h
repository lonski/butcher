...
namespace permissive
{
  ...
  // {
  // public:
  //   bool isOpaque(short destX, short destY);
  ...
  {
  public:
    static int isOpaque(short destX, short destY, void * context)
    {
      T * typedContext = reinterpret_cast<T *>(context);
      return typedContext->isOpaque(destX, destY);
    }
  ...
  void fov(short sourceX, short sourceY, maskT & mask, T & context)
  {
    permissiveFov(sourceX, sourceY, mask.getMask(),
                  fovPrivateT<T>::isOpaque, fovPrivateT<T>::visit,
                  reinterpret_cast<void *>(&context));
  }
}
